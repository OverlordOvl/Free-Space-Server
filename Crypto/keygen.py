...
def create_certificate(
    req, issuerCertKey, serial, validityPeriod,
    digest="sha256"
    ):
    """
...
if __name__ == "__main__":
    ...
    cacert = create_certificate(
        careq, (careq, cakey), 0, (0, 60 * 60 * 24 * 365)
        )  # one year
    open('clientkey.pem', 'w').write(
        crypto.dump_privatekey(
            crypto.FILETYPE_PEM, cakey, passphrase='12345678'
            )
        )
    open('clientcert.pem', 'w').write(
